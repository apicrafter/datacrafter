...
def get_source_from_file(filename, stype=None, options=None):
    logging.info('Getting source from extractor results, filename: %s stype: %s, options: %s' % (str(filename), str(stype), str(options)))
    ...
    if stype == 'zipxml':
        ...
        return XLSSource(filename=filename, keys=keys, start_line=options['start_line'] if 'start_line' in options.keys() else 0)
    elif stype == 'xlsx':
        validate_options(options, ['keys',])
        if 'keys' in options.keys() and options['keys']:
            keys = options['keys'].split(',')
        ...
        return XLSXSource(filename=filename, keys=keys, start_line=options['start_line'] if 'start_line' in options.keys() else 0)
    elif stype == 'csv':
        ...
        logging.debug('Use CSV source with filename %s, keys %s, delimiter "%s", encoding %s' % (filename, keys, delimiter, encoding))
