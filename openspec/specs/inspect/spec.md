...
## Purpose
Inspect a project without running a full load: dry-run plans, inferred schemas, and field metrics from JSONL.
## Requirements
...
#### Scenario: dry-run lists every extractor
- **WHEN** a project uses `extractors:` with two items
- **THEN** the dry-run plan includes both extractor types (and keeps `extractor` as the first spec for compatibility)
